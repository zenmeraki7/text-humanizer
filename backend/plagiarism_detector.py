...
"""
Plagiarism Detection and Removal Module
Loads patterns from external files (JSON and Excel)
"""
...
class PlagiarismDetector:
    def __init__(self, anthropic_api_key: str, ai_detector=None):
        ...
        self.ai_patterns = self._load_patterns_from_json()
        self.excel_patterns = self._load_patterns_from_excel()
        
        logger.info("✅ Plagiarism Detector initialized with external patterns")

    def _load_patterns_from_json(self):
        """Load AI patterns from JSON file"""
        try:
            with open('ai_patterns.json', 'r') as f:
                patterns = json.load(f)
            logger.info(f"✅ Loaded AI patterns from JSON: {len(patterns)} categories")
            return patterns
        except FileNotFoundError:
            logger.warning("⚠️ ai_patterns.json not found, using AI detector patterns")
            if self.ai_detector and hasattr(self.ai_detector, 'ai_patterns'):
                return self.ai_detector.ai_patterns
            return {}
        except Exception as e:
            logger.error(f"❌ Error loading JSON patterns: {e}")
            return {}

    def _load_patterns_from_excel(self):
        """Load patterns from Excel file"""
        try:
            df = pd.read_excel('phrasal_patterns.xlsx')
            patterns = {}
            
            # Process Excel columns (simplified extraction)
            for col in df.columns:
                col_patterns = df[col].dropna().astype(str).str.strip()
                col_patterns = [p for p in col_patterns if p and len(p) > 2]
                if col_patterns:
                    patterns[f'excel_{col.lower()}'] = col_patterns[:50]  # Limit patterns
            
            logger.info(f"✅ Loaded Excel patterns: {len(patterns)} categories")
            return patterns
        except FileNotFoundError:
    ...
    def flag_patterns(self, text: str) -> Dict[str, List[str]]:
        """Flag detected patterns in text"""
        flagged = {}
        ...
        for category, patterns in self.ai_patterns.items():
            ...
            if detected:
                flagged[category] = detected[:5]  # Limit to 5 per category
        
        # Check Excel patterns
        for category, patterns in self.excel_patterns.items():
            detected = []
            for pattern in patterns:
                if isinstance(pattern, str) and pattern.lower() in text_lower:
                    detected.append(pattern)
            if detected:
                flagged[category] = detected[:3]  # Limit to 3 per category
        
        return flagged

    def _call_anthropic_with_retry(self, prompt: str, max_retries=3):
            ...
            try:
                ...
                if "overloaded" in error_str or "529" in error_str:
                    if attempt < max_retries - 1:
                        ...
                        raise HTTPException(status_code=503, detail="Service temporarily overloaded. Please try again in a few minutes.")
                else:
                    raise e

    def remove_plagiarism(self, text: str, rewrite_mode: str = "balanced", reference_text: str = "") -> dict:
        """Remove plagiarism from text using AI rewriting"""
        if not self.anthropic_available:
            return {"error": "Plagiarism removal requires ANTHROPIC_API_KEY"}
        ...
        try:
            ...
            flagged_patterns = self.flag_patterns(text)
            flagged_list = []
            for patterns in flagged_patterns.values():
                flagged_list.extend(patterns)

            # Create rewriting prompt based on mode
            mode_prompts = {
                "conservative": "Lightly rewrite this text to avoid plagiarism while keeping the original structure and meaning.",
                "balanced": "Rewrite this text to eliminate plagiarism by changing vocabulary and sentence structure while preserving the meaning.",
                "aggressive": "Completely rewrite this text from scratch to eliminate all plagiarism while keeping the same information."
            }

            base_prompt = mode_prompts.get(rewrite_mode, mode_prompts["balanced"])
...
Instructions:
- Write naturally and avoid AI patterns
...
Return only the rewritten text."""
            ...
            # Add flagged patterns warning
            if flagged_list:
                prompt += f"\n\nAvoid these detected patterns: {', '.join(flagged_list[:10])}"

            # Get rewritten text with retry logic
            rewritten_text = self._call_anthropic_with_retry(prompt)

            # Calculate basic metrics
            original_words = len(text.split())
            new_words = len(rewritten_text.split())
            
            # Estimate improvement based on flagged patterns removed
            patterns_before = len(flagged_list)
            patterns_after = len(self.flag_patterns(rewritten_text))
            improvement = max(0, (patterns_before - patterns_after) * 10)
            ...
            return {
                ...
                "ai_improvement": max(0, patterns_before - patterns_after) * 5,
                "original_plagiarism_score": 0,
                "new_plagiarism_score": 0,
                "length_change": ((new_words - original_words) / original_words * 100) if original_words > 0 else 0,
                "rewrite_mode": rewrite_mode,
                "strategy_description": mode_prompts.get(rewrite_mode, "Standard rewriting"),
                "patterns_flagged": patterns_before,
