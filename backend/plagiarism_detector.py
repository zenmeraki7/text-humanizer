...
#         """Call Anthropic API with retry logic for overload errors"""
...
#                         flagged_phrases.extend(patterns)
#                 except:
#                     # Continue without AI patterns if detector fails
#                     pass
...
#             # ✅ FIXED: Use retry logic instead of direct API call
#             rewritten_text = self._call_anthropic_with_retry(full_prompt)
...
#                     new_ai_score, _ = self.ai_detector.calculate_ai_score(rewritten_text)
#                 except:
#                     # Continue without AI scoring if detector fails
#                     pass
...
class PlagiarismDetector:
    def __init__(self, anthropic_api_key: str = None, ai_detector=None):
        ...
        If no API key is passed, it will try to read ANTHROPIC_API_KEY from environment variables.
        """
        # Load key from env if not passed
        anthropic_api_key = anthropic_api_key or os.getenv("ANTHROPIC_API_KEY", "dummy_key")
        ...
        if self.anthropic_available:
            ...
            self.client = None
            logger.warning("⚠️ Anthropic client disabled for plagiarism removal (no API key)")
        ...
        try:
            ...
            if self.ai_detector:
                try:
                    ...
                    pass  # continue without AI patterns
...
Return only the rewritten version with no explanations."""
            ...
            # AI score improvement (if AI detector available)
            original_ai_score = 0.0
            new_ai_score = 0.0
            if self.ai_detector:
                try:
