...
"""
AI Text Detection Module with Enhanced Chunking Support
Contains all AI pattern detection and scoring functionality
"""
...
class AITextDetector:
    ...
    def chunk_text(self, text: str, max_chunk_size: int = 3000) -> List[str]:
        """Split text into chunks for processing large texts without losing context"""
    ...
    def detect_ai_patterns(self, text: str) -> Dict[str, List[str]]:
        ...
        for category, patterns in self.ai_patterns.items():
            ...
            # Handle rhetorical questions separately
            if category == 'rhetorical_questions':
                detected[category] = []
                for subcategory, questions in patterns.items():
                    for question in questions:
                        if re.search(r'\b' + re.escape(question.lower()) + r'\b', text_lower):
                            detected[category].append(question)
                continue
    ...
    def analyze_large_text(self, text: str) -> Tuple[float, dict]:
        """Handle large texts by chunking and combining results"""
        if len(text) <= 5000:  # Process normally for smaller texts
            return self.calculate_ai_score(text)
        
        logger.info(f"📄 Processing large text ({len(text)} chars) in chunks...")
        ...
        for i, chunk in enumerate(chunks):
            try:
                chunk_score, chunk_details = self.calculate_ai_score(chunk)
                chunk_results.append(chunk_score)
                total_patterns += chunk_details.get('total_patterns', 0)
                
                # Collect detected patterns for combined report
                chunk_patterns = self.detect_ai_patterns(chunk)
                for category, patterns in chunk_patterns.items():
                    if category not in combined_patterns:
                        combined_patterns[category] = set()
        ...
        if not chunk_results:
            return 0.0, {"error": "All chunks failed to process"}
        
        # Calculate weighted average (give more weight to higher scores)
        sorted_scores = sorted(chunk_results)
        if len(sorted_scores) >= 3:
            # Use top 60% of scores for final calculation
            top_scores = sorted_scores[int(len(sorted_scores) * 0.4):]
            final_score = sum(top_scores) / len(top_scores)
        else:
            final_score = sum(chunk_results) / len(chunk_results)
        
        # Convert sets back to lists for JSON serialization
        combined_patterns = {k: list(v) for k, v in combined_patterns.items()}
        
        combined_details = {
            'total_chunks': len(chunks),
            'chunk_scores': [{'chunk': i+1, 'score': score} for i, score in enumerate(chunk_results)],
            'word_count': len(text.split()),
            'combined_patterns': combined_patterns,
            'total_patterns': total_patterns,
            'pattern_density': (total_patterns / len(text.split())) * 100,
            'avg_chunk_score': sum(chunk_results) / len(chunk_results),
            'final_score': final_score,
            'structure_score': 0.5,  # Average for large texts
            'structure_details': f"Large text processed in {len(chunks)} chunks"
        }
        
        logger.info(f"✅ Processed {len(chunks)} chunks, final score: {final_score:.1f}%")
        return final_score, combined_details

    def get_detection_report(self, text: str) -> Tuple[str, float, str]:
        """Generate comprehensive AI detection report with chunking support"""
        # Automatically handle large texts
        if len(text) > 5000:
            ai_score, details = self.analyze_large_text(text)
            return self._generate_large_text_report(ai_score, details, text)
        else:
            ai_score, details = self.calculate_ai_score(text)
            detected_patterns = self.detect_ai_patterns(text)
            return self._generate_standard_report(ai_score, details, detected_patterns)

    def _generate_standard_report(self, ai_score: float, details: dict, detected_patterns: dict) -> Tuple[str, float, str]:
        """Generate report for standard-sized texts"""
        # Determine classification
        if ai_score >= 70:
            classification = "🤖 **HIGHLY LIKELY AI-GENERATED**"
            confidence = "Very High"
            ...
            for category, patterns in detected_patterns.items():
                if patterns:
                    ...
                    for i, pattern in enumerate(patterns[:3]):
                        ...
                        # Add suggestions if available
                        if hasattr(self, 'pattern_details') and pattern.lower() in self.pattern_details:
                            details_obj = self.pattern_details[pattern.lower()]
                            if details_obj['suggestions']:
                                suggestions = ', '.join(details_obj['suggestions'][:2])  # Limit to 2 suggestions
                                severity_emoji = {"high": "🔴", "medium": "🟡", "low": "🟢"}.get(details_obj['severity'], "⚪")
                                pattern_info += f" {severity_emoji} → {suggestions}"
    ...
    def _generate_large_text_report(self, ai_score: float, details: dict, original_text: str) -> Tuple[str, float, str]:
        """Generate report for large texts with chunk analysis"""
        # Determine classification
        if ai_score >= 70:
            classification = "🤖 **HIGHLY LIKELY AI-GENERATED**"
            confidence = "Very High"
        ...
        # Add chunk analysis for large texts
        report += f"**📄 Large Text Analysis:**\n"
        report += f"• Processed in {details['total_chunks']} chunks\n"
        report += f"• Average chunk score: {details.get('avg_chunk_score', 0):.1f}%\n"
        
        # Show chunk score distribution
        chunk_scores = [c['score'] for c in details.get('chunk_scores', [])]
        if chunk_scores:
            report += f"• Score range: {min(chunk_scores):.1f}% - {max(chunk_scores):.1f}%\n"
        report += "\n"
        
        # Show combined patterns
        combined_patterns = details.get('combined_patterns', {})
        if combined_patterns:
            total_flags = sum(len(patterns) for patterns in combined_patterns.values())
            report += f"**🚩 {total_flags} AI PATTERNS DETECTED ACROSS ALL CHUNKS:**\n\n"
            ...
            for category, patterns in combined_patterns.items():
                if patterns:
                    ...
                    count = len(patterns)
                    report += f"• **{name}** ({count}): "
                    pattern_list = list(patterns)[:3]  # Show first 3
                    report += ", ".join(f'"{p}"' for p in pattern_list)
                    if len(patterns) > 3:
                        report += f" +{len(patterns)-3} more"
                    report += "\n"
        else:
            report += "✅ **No common AI patterns detected across chunks**\n"
        
        # Technical details
        report += f"\n**📊 Technical Analysis:**\n"
        report += f"• Total word count: {details.get('word_count', 0)}\n"
        report += f"• Pattern density: {details.get('pattern_density', 0):.2f} per 100 words\n"
        report += f"• {details.get('structure_details', 'Structure analysis averaged across chunks')}\n"
        
        return report, ai_score, classification

    def detect_text(self, text: str) -> Tuple[str, float, str]:
        """Main detection method that automatically handles both small and large texts"""
        return self.get_detection_report(text)
