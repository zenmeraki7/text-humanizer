...
app = FastAPI(title="AI Text Analyzer API", version="1.0")
...
async def root():
    return {
        "message": "Welcome to the AI Text Analyzer API",
        "available_endpoints": [
            "/humanize",
            "/analyze",
            "/detect-plagiarism",
...
# ✅ HUMANIZE ENDPOINT
@app.post("/humanize")
async def humanize_text(data: TextInput):
    try:
        input_text = data.text.strip()
        if not input_text:
            raise HTTPException(status_code=400, detail="Input text cannot be empty")

        # Process text using Humanizer class
        humanized_output = humanizer.humanize_text(input_text)

        return {
            "success": True,
            "original_text": input_text,
            "humanized_text": humanized_output,
        }

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error in humanizing text: {str(e)}")

# ✅ EXAMPLE: PLAGIARISM DETECTION ENDPOINT
@app.post("/detect-plagiarism")
async def detect_plagiarism(data: TextInput):
    try:
        input_text = data.text.strip()
        if not input_text:
            raise HTTPException(status_code=400, detail="Input text cannot be empty")

        result = plagiarism_detector.detect_plagiarism_patterns(input_text)
        return {"success": True, "plagiarism_result": result}

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error detecting plagiarism: {str(e)}")

# ✅ HEALTH CHECK
@app.get("/api-status")
async def api_status():
    return {"status": "running", "service": "AI Text Analyzer", "version": "1.0"}
