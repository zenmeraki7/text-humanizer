...
try:
    ...
    # Get API key - now should work with .env file
    api_key = os.getenv("ANTHROPIC_API_KEY", "dummy_key")
    
    # Log API key status for debugging
    if api_key == "dummy_key":
        logger.warning("⚠️ Using dummy API key - check environment variables")
    else:
...
async def debug():
    ...
    api_key = os.getenv("ANTHROPIC_API_KEY", "dummy api")
    return {
...
async def plagiarism_check(request: PlagiarismCheckRequest):
    ...
    if modules_loaded:
        try:
            ...
            logger.error(f"Error in plagiarism_check: {str(e)}")
            raise HTTPException(status_code=500, detail=str(e))
    else:
        # Basic similarity check
        from difflib import SequenceMatcher
...
# Fixed endpoint path to match frontend
@app.post("/remove-plagiarism")
async def remove_plagiarism(request: PlagiarismRemoveRequest):
    """Remove plagiarism from text - endpoint matching frontend"""
    return await plagiarism_remove(request)
...
async def plagiarism_remove(request: PlagiarismRemoveRequest):
    ...
    if modules_loaded:
        try:
            ...
            logger.error(f"Error in plagiarism_remove: {str(e)}")
            raise HTTPException(status_code=500, detail=str(e))
    else:
        # Basic text cleaning
        text = request.text
