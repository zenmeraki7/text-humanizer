...
// export const API_BASE_URL = "https://test-69jq.onrender.com";
...
// ✅ FIXED: Uniqueness score calculation - directly use backend score
export const getUniquenessScore = (apiResult) => {
  // If backend provides uniqueness_score, use it directly
  if (apiResult && typeof apiResult.uniqueness_score === 'number') {
    return Math.round(apiResult.uniqueness_score);
  }
  
  // Fallback: Try to extract from other metrics if uniqueness_score is missing
  if (apiResult) {
    // If we have improvement data, calculate based on that
    const plagiarismImprovement = apiResult.improvement || 0;
    const aiImprovement = apiResult.ai_improvement || 0;
    
    // Start from a reasonable baseline and add improvements
    const baseScore = 50; // More realistic baseline
    const calculatedScore = baseScore + (plagiarismImprovement * 0.5) + (aiImprovement * 0.3);
    return Math.min(99, Math.max(0, Math.round(calculatedScore)));
  }
